...
CsvMapping = (function() {
  ...
  CsvMapping.prototype.parse = function(csvString) {
    var deferred;
    deferred = Q.defer();
    Csv().from.string(csvString).on('error', function(error) {
  ...
  CsvMapping.prototype.toCSV = function(header, data) {
    var deferred;
    deferred = Q.defer();
    Csv().from([header].concat(data)).to.string(function(asString) {
      return deferred.resolve(asString);
